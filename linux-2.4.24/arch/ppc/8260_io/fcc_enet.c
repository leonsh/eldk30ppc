...
/* hwuang, 11/18/04
 * FCC1, CLK11 is the receive clock, CLK12 is the transmit clock
 */
#if defined(CONFIG_ONU8260) || defined(CONFIG_OLC8260)
...
#endif	/* CONFIG_PM826 */
#endif  /* CONFIG_ONU8260 or CONFIG_OLC8260 */
...
mii_discover_phy3(uint mii_reg, struct net_device *dev)
{
	...
	if(!phy_info[i]) {
		if (fep->phy_id == 0xffff) {
			printk ("%s: no PHY found - interface inoperative\n",
				dev->name);
		} else {
			printk ("%s: PHY id 0x%08x is not supported!\n",
				dev->name, fep->phy_id);
		}
		return;
	}
...
int __init fec_enet_init(void)
{
	...
	while (np-- > 0) {
		...
		*/
		...
		printk("%s: FCC%d ENET Version 0.4, ",
			dev->name, fip->fc_fccnum + 1);
		for (i=0; i<5; i++)
						...
						volatile immap_t *immap)
{
	...
	*/
	 ...
	 */
	*eap++ = dev->dev_addr[5] = bd->bi_enetaddr[5] + fip->fc_fccnum;
	for (i=4; i>=0; i--) {
...
#if defined (CONFIG_USE_MDIO) && !defined (CONFIG_PM826)
...
# endif
...
#endif	/* CONFIG_USE_MDIO, CONFIG_PM826 */

	/* Set GFMR to enable Ethernet operating mode.
	 */
	 // hwuang, 10/28/04, need porting effort for OLC8260
	...
	*/
	...
	fcc_restart (dev, 1);
/* hwuang, 11/18/04 */
#elif defined(CONFIG_OLC8260) || defined(CONFIG_ONU8260)
	/* start in full duplex mode, and negotiate speed */
	fcc_restart (dev, 1);
#else
