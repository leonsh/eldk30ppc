/*
 *  UART driver for MPC8xx CPM SCC or SMC
...
#if defined(CONFIG_UART_CTS_CONTROL_SCC1) || \
    defined(CONFIG_UART_RTS_CONTROL_SCC1) || \
    defined(CONFIG_UART_CD_CONTROL_SCC1)  || \
    defined(CONFIG_UART_DTR_CONTROL_SCC1) || \
    defined(CONFIG_UART_CTS_CONTROL_SCC2) || \
    defined(CONFIG_UART_RTS_CONTROL_SCC2) || \
    defined(CONFIG_UART_CD_CONTROL_SCC2)  || \
    defined(CONFIG_UART_DTR_CONTROL_SCC2) || \
    defined(CONFIG_UART_CTS_CONTROL_SCC3) || \
    defined(CONFIG_UART_RTS_CONTROL_SCC3) || \
    defined(CONFIG_UART_CD_CONTROL_SCC3)  || \
    defined(CONFIG_UART_DTR_CONTROL_SCC3) || \
    defined(CONFIG_UART_CTS_CONTROL_SCC4) || \
    defined(CONFIG_UART_RTS_CONTROL_SCC4) || \
    defined(CONFIG_UART_CD_CONTROL_SCC4)  || \
    defined(CONFIG_UART_DTR_CONTROL_SCC4)
...
#if (CONFIG_UART_DTR_CONTROL_SCC1 == PORT_A)
...
#endif
#if (CONFIG_UART_DTR_CONTROL_SCC1 == PORT_D)
...
#endif
#if (CONFIG_UART_DTR_CONTROL_SCC2 == PORT_A)
...
#endif
#if (CONFIG_UART_DTR_CONTROL_SCC2 == PORT_D)
...
#endif
#if (CONFIG_UART_DTR_CONTROL_SCC3 == PORT_A)
...
#endif
#if (CONFIG_UART_DTR_CONTROL_SCC3 == PORT_D)
...
#endif
#if (CONFIG_UART_DTR_CONTROL_SCC4 == PORT_A)
...
#endif
#if (CONFIG_UART_DTR_CONTROL_SCC4 == PORT_D)
...
#endif
#ifdef CONFIG_UART_DTR_CONTROL_SCC1
#if (CONFIG_UART_DTR_CONTROL_SCC1 != PORT_B)
#define DTR1_PIN (1 << (15 - CONFIG_DTR1_PIN))
#endif
#endif
#ifdef CONFIG_UART_DTR_CONTROL_SCC2
#if (CONFIG_UART_DTR_CONTROL_SCC2 != PORT_B)
#define DTR2_PIN (1 << (15 - CONFIG_DTR2_PIN))
#endif
#endif
#ifdef CONFIG_UART_DTR_CONTROL_SCC3
#if (CONFIG_UART_DTR_CONTROL_SCC3 != PORT_B)
#define DTR3_PIN (1 << (15 - CONFIG_DTR3_PIN))
#endif
#endif
#ifdef CONFIG_UART_DTR_CONTROL_SCC4
#if (CONFIG_UART_DTR_CONTROL_SCC4 != PORT_B)
#define DTR4_PIN (1 << (15 - CONFIG_DTR4_PIN))
#endif
#endif
...
/* SCC2 for console, no other SMCx or SCCx used here
*/
...
#ifdef CONFIG_SCC1_UART_TX_BDNUM
  CONFIG_SCC1_UART_TX_BDNUM,
#else
  0,
#endif
#ifdef CONFIG_SCC2_UART_TX_BDNUM
  CONFIG_SCC2_UART_TX_BDNUM,
#else
  0,
#endif
#ifdef CONFIG_SCC3_UART_TX_BDNUM
  CONFIG_SCC3_UART_TX_BDNUM,
#else
  0,
#endif
#ifdef CONFIG_SCC4_UART_TX_BDNUM
  CONFIG_SCC4_UART_TX_BDNUM,
#else
  0,
#endif
#ifdef CONFIG_SMC1_UART_TX_BDNUM
  CONFIG_SMC1_UART_TX_BDNUM,
#else
  0,
#endif
#ifdef CONFIG_SMC2_UART_TX_BDNUM
  CONFIG_SMC2_UART_TX_BDNUM
#else
  0
#endif
};
...
#ifdef CONFIG_SCC1_UART_RX_BDSIZE
  CONFIG_SCC1_UART_RX_BDSIZE,
#else
  0,
#endif
#ifdef CONFIG_SCC2_UART_RX_BDSIZE
  CONFIG_SCC2_UART_RX_BDSIZE,
#else
  0,
#endif
#ifdef CONFIG_SCC3_UART_RX_BDSIZE
  CONFIG_SCC3_UART_RX_BDSIZE,
#else
  0,
#endif
#ifdef CONFIG_SCC4_UART_RX_BDSIZE
  CONFIG_SCC4_UART_RX_BDSIZE,
#else
  0,
#endif
#ifdef CONFIG_SMC1_UART_RX_BDSIZE
  CONFIG_SMC1_UART_RX_BDSIZE,
#else
  0,
#endif
#ifdef CONFIG_SMC2_UART_RX_BDSIZE
  CONFIG_SMC2_UART_RX_BDSIZE
#else
  0
#endif
};
...
#ifdef CONFIG_SCC1_UART_TX_BDSIZE
  CONFIG_SCC1_UART_TX_BDSIZE,
#else
  0,
#endif
#ifdef CONFIG_SCC2_UART_TX_BDSIZE
  CONFIG_SCC2_UART_TX_BDSIZE,
#else
  0,
#endif
#ifdef CONFIG_SCC3_UART_TX_BDSIZE
  CONFIG_SCC3_UART_TX_BDSIZE,
#else
  0,
#endif
#ifdef CONFIG_SCC4_UART_TX_BDSIZE
  CONFIG_SCC4_UART_TX_BDSIZE,
#else
  0,
#endif
#ifdef CONFIG_SMC1_UART_TX_BDSIZE
  CONFIG_SMC1_UART_TX_BDSIZE,
#else
  0,
#endif
#ifdef CONFIG_SMC2_UART_TX_BDSIZE
  CONFIG_SMC2_UART_TX_BDSIZE
#else
  0
#endif
};
...
static int startup(ser_info_t *info)
{
	...
	if (PORT_IS_SCC(info->state->smc_scc_num)) {
		...
		scup->scc_genscc.scc_mrblr = rx_bd_sizes[idx];

		scup->scc_maxidl = rx_bd_sizes[idx];

#ifdef CONFIG_UART_MAXIDL_SCC1
		if ( idx == 0 )
			scup->scc_maxidl = CONFIG_UART_MAXIDL_SCC1;
#endif
#ifdef CONFIG_UART_MAXIDL_SCC2
		if ( idx == 1 )
			scup->scc_maxidl = CONFIG_UART_MAXIDL_SCC2;
#endif
#ifdef CONFIG_UART_MAXIDL_SCC3
		if ( idx == 2 )
			scup->scc_maxidl = CONFIG_UART_MAXIDL_SCC3;
#endif
#ifdef CONFIG_UART_MAXIDL_SCC4
		if ( idx == 3 )
			scup->scc_maxidl = CONFIG_UART_MAXIDL_SCC4;
#endif
		...
		*/
		...
		up->smc_mrblr = rx_bd_sizes[idx+4];

		up->smc_maxidl = rx_bd_sizes[idx+4];

#ifdef CONFIG_UART_MAXIDL_SMC2
		if ( idx == 1 )
			up->smc_maxidl = CONFIG_UART_MAXIDL_SMC2;
#endif
...
static void change_speed(ser_info_t *info)
{
	...
	if (PORT_IS_SCC(state->smc_scc_num)) {
		idx = PORT_NUM(state->smc_scc_num);
#ifdef CONFIG_UART_CTS_CONTROL_SCC1
		if ( idx == 0 )
			scval = SCU_PMSR_FLC;
#endif
#ifdef CONFIG_UART_CTS_CONTROL_SCC2
		if ( idx == 1 )
			scval = SCU_PMSR_FLC;
#endif
#ifdef CONFIG_UART_CTS_CONTROL_SCC3
		if ( idx == 2 )
			scval = SCU_PMSR_FLC;
#endif
#ifdef CONFIG_UART_CTS_CONTROL_SCC4
		if ( idx == 3 )
			scval = SCU_PMSR_FLC;
#endif
	}
	...
	info->timeout = (tx_bd_sizes[ PORT_IS_SCC(info->state->smc_scc_num)  ?
	                              PORT_NUM(info->state->smc_scc_num)     :
	                              PORT_NUM(info->state->smc_scc_num) + 4
	                            ]*HZ*bits);
		    ...
		    const unsigned char *buf, int count)
{
	...
	tx_buf_size = tx_bd_sizes[ PORT_IS_SCC(info->state->smc_scc_num)  ?
	                           PORT_NUM(info->state->smc_scc_num)     :
	                           PORT_NUM(info->state->smc_scc_num) + 4 ];
	while (1) {
...
static int rs_8xx_write_room(struct tty_struct *tty)
{
	...
	if ((info->tx_cur->cbd_sc & BD_SC_READY) == 0) {
		info->flags &= ~TX_WAKEUP;
		ret = tx_bd_sizes[ PORT_IS_SCC(info->state->smc_scc_num)  ?
		                   PORT_NUM(info->state->smc_scc_num)     :
		                   PORT_NUM(info->state->smc_scc_num) + 4 ];
	}
	else {
...
/*		current->counter = 0;	 make us low-priority */
		 ...
		 */
		local_irq_save(flags);
		bd_num = tx_bd_nums[ PORT_IS_SCC(info->state->smc_scc_num)  ?
		                     PORT_NUM(info->state->smc_scc_num)     :
		                     PORT_NUM(info->state->smc_scc_num) + 4 ];
		bdp = info->tx_cur;
				...
				unsigned count)
{
	...
	if ((info = (ser_info_t *)ser->info) != NULL) {
		...
		if (PORT_IS_SCC(ser->smc_scc_num)) {
			...
			sup = (scc_uart_t *)&cpmp->cp_dparam[ser->port];

			/* Get the address of the host memory buffer.*/
			info->tx_bd_base = (cbd_t *)bdbase = (cbd_t *)&cpmp->cp_dpmem[sup->scc_genscc.scc_tbase];
		} else {
			/* Pointer to UART in parameter ram. */
...
static int my_console_wait_key(int idx, int xmon, char *obuf)
{
	...
	*/
	...
	if ((info = (ser_info_t *)ser->info)) {
		...
		if (PORT_IS_SCC(ser->smc_scc_num)) {
			bdp = (cbd_t *)&cpmp->cp_dpmem[sup->scc_genscc.scc_rbase];
		} else {
...
int __init rs_8xx_init(void)
{
	 ...
	 */
	iobits = 0
#if defined(CONFIG_SCC1_UART)
		| 0x0003
#endif
#if defined(CONFIG_SCC2_UART)
		| 0x000C
#endif
#if defined(CONFIG_SCC3_UART)	&& \
   !defined(CONFIG_MBX)		&& \
   ...
   !defined(CONFIG_SM850)
		| 0x0030
#endif
#if defined(CONFIG_SCC4_UART) && !defined(CONFIG_IP860)
		| 0x00C0
#endif
		;
...
# endif
	...
	immap->im_ioport.iop_pcdir &= ~(1 << (15 - CONFIG_RTS1_PIN));
#endif
#if (CONFIG_UART_RTS_CONTROL_SCC2 == PORT_C)
	immap->im_ioport.iop_pcpar |=  (1 << (15 - CONFIG_RTS2_PIN));
	immap->im_ioport.iop_pcdir &= ~(1 << (15 - CONFIG_RTS2_PIN));
#endif
#if (CONFIG_UART_RTS_CONTROL_SCC3 == PORT_C)
	immap->im_ioport.iop_pcpar |=  (1 << (15 - CONFIG_RTS3_PIN));
	immap->im_ioport.iop_pcdir &= ~(1 << (15 - CONFIG_RTS3_PIN));
#endif
#if (CONFIG_UART_RTS_CONTROL_SCC4 == PORT_C)
	immap->im_ioport.iop_pcpar |=  (1 << (15 - CONFIG_RTS4_PIN));
	immap->im_ioport.iop_pcdir &= ~(1 << (15 - CONFIG_RTS4_PIN));
#endif
#if (CONFIG_UART_RTS_CONTROL_SCC3 == PORT_D)
	immap->im_ioport.iop_pdpar |=  (1 << (15 - CONFIG_RTS3_PIN));
#endif
#if (CONFIG_UART_RTS_CONTROL_SCC4 == PORT_D)
	immap->im_ioport.iop_pdpar |=  (1 << (15 - CONFIG_RTS4_PIN));
#endif
        iobits = 0
#if defined(CONFIG_UART_CTS_CONTROL_SCC1)
		| 0x0010
#endif
#if defined(CONFIG_UART_CD_CONTROL_SCC1)
		| 0x0020
#endif
#if defined(CONFIG_UART_CTS_CONTROL_SCC2)
		| 0x0040
#endif
#if defined(CONFIG_UART_CD_CONTROL_SCC2)
		| 0x0080
#endif
#if defined(CONFIG_UART_CTS_CONTROL_SCC3)
		| (1 << (15 - CONFIG_CTS3_PIN))
#endif
#if defined(CONFIG_UART_CD_CONTROL_SCC3)
		| (1 << (15 - CONFIG_CD3_PIN))
#endif
#if defined(CONFIG_UART_CTS_CONTROL_SCC4)
		| 0x0400
#endif
#if defined(CONFIG_UART_CD_CONTROL_SCC4)
		| 0x0800
#endif
		;
	immap->im_ioport.iop_pcpar &= ~iobits;
...
#if defined(CONFIG_UART_DTR_CONTROL_SCC1)
	PORT_DTR1_PAR &= ~DTR1_PIN;	/* DTR1 */
	PORT_DTR1_DAT |=  DTR1_PIN;
	PORT_DTR1_DIR |=  DTR1_PIN;
#endif
#if defined(CONFIG_UART_DTR_CONTROL_SCC2)
	PORT_DTR2_PAR &= ~DTR2_PIN;	/* DTR2 */
	PORT_DTR2_DAT |=  DTR2_PIN;
	PORT_DTR2_DIR |=  DTR2_PIN;
#endif
#if defined(CONFIG_UART_DTR_CONTROL_SCC3)
	PORT_DTR3_PAR &= ~DTR3_PIN;	/* DTR3 */
	PORT_DTR3_DAT |=  DTR3_PIN;
	PORT_DTR3_DIR |=  DTR3_PIN;
#endif
#if defined(CONFIG_UART_DTR_CONTROL_SCC4)
	...
	*/
	...
	for (i = 0, state = rs_table; i < NR_PORTS; i++,state++) {
		...
		if (info) {
			...
			dp_addr = m8xx_cpm_dpalloc(sizeof(cbd_t) *
			            rx_bd_nums[ PORT_IS_SCC(state->smc_scc_num) ?
			                        PORT_NUM(state->smc_scc_num)    :
			                        PORT_NUM(state->smc_scc_num) + 4]
			          );
			...
			*/
			...
			mem_addr = m8xx_cpm_hostalloc(
			             rx_bd_nums[ PORT_IS_SCC(state->smc_scc_num) ?
			                         PORT_NUM(state->smc_scc_num)    :
			                         PORT_NUM(state->smc_scc_num) + 4  ] *
			             rx_bd_sizes[PORT_IS_SCC(state->smc_scc_num) ?
			                         PORT_NUM(state->smc_scc_num)    :
			                         PORT_NUM(state->smc_scc_num) + 4  ]
			           );
			...
			for (j=0;
				 j<rx_bd_nums[ PORT_IS_SCC(state->smc_scc_num)  ?
				               PORT_NUM(state->smc_scc_num)     :
				               PORT_NUM(state->smc_scc_num)+4]-1;
				 j++) {
				bdp->cbd_bufaddr = __pa(mem_addr);
				bdp->cbd_sc = BD_SC_EMPTY | BD_SC_INTRPT;
				mem_addr += rx_bd_sizes[PORT_IS_SCC(state->smc_scc_num) ?
				                        PORT_NUM(state->smc_scc_num)    :
				                        PORT_NUM(state->smc_scc_num) + 4 ];
				bdp++;
			}
			...
			dp_addr = m8xx_cpm_dpalloc(sizeof(cbd_t) *
			            tx_bd_nums[ PORT_IS_SCC(info->state->smc_scc_num)  ?
			                        idx : idx + 4 ]
			          );
			...
			*/
			mem_addr = m8xx_cpm_hostalloc(
			             tx_bd_nums[ PORT_IS_SCC(info->state->smc_scc_num) ?
			                         idx : idx + 4 ] *
			             tx_bd_sizes[PORT_IS_SCC(info->state->smc_scc_num) ?
			                         idx : idx + 4 ]
			           );
			...
			for (j=0;
				 j<tx_bd_nums[PORT_IS_SCC(info->state->smc_scc_num) ?
				               idx : idx + 4 ] - 1;
				 j++) {
				bdp->cbd_bufaddr = __pa(mem_addr);
				bdp->cbd_sc = BD_SC_INTRPT;
				mem_addr += tx_bd_sizes[PORT_IS_SCC(info->state->smc_scc_num) ?
				                        idx : idx + 4 ];
				bdp++;
			}
			bdp->cbd_bufaddr = iopa(mem_addr);
			...
			if (PORT_IS_SCC(info->state->smc_scc_num)) {
				 ...
				 */
				iobits = 0x000000FF << (idx*8);	/* clear GRx, SCx */
				cp->cp_sicr &= ~iobits;
				iobits  = (PORT_BRG(info->state->smc_scc_num) &
					   0x03) << 3;	/* Rx BRG */
				iobits |= (PORT_BRG(info->state->smc_scc_num) &
					   0x03);	/* Tx BRG */
				iobits <<= (idx*8);
				...
				*/
				...
				cp->cp_simode |=
				  (PORT_BRG(state->smc_scc_num) << ((idx * 16) + 12));
...
static int __init serial_console_setup(struct console *co, char *options)
{
	...
	*/
	...
	/* Set up the baud rate generator.
	*/
	m8xx_cpm_setbrg(PORT_BRG(ser->smc_scc_num), bd->bi_baudrate);
