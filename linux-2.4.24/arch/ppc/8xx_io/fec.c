 ...
 *
 * Notice that the mii_parse_* functions can potentially mask slight hardware
 * timing issues, which is why we do uint s = fep->phy_status, not
...
fec_enet_open(struct net_device *dev)
{
	...
	if (fep->phy) {
		...
		while(fep->link == 0 ||
                      ((((volatile fec_t*)dev->base_addr)->fec_ecntrl &
                       FEC_ECNTRL_ETHER_EN) == 0))
		{
			schedule();
		}
#endif /* CONFIG_IP_PNP */
...
# ifdef CONFIG_TQM8xxL
	/*
	 * TQM8xxL/M use sequential MAC addresses
	 */
	tmpaddr[5] += dev->name[3] - '0';	/* "ethX" -> 'X' */
# else
	tmpaddr[3] |= 0x80;
# endif
	iap = tmpaddr;
