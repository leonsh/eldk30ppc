...
/*
 * GUI generates a set #defines per configured task to 
                       ...
                       TaskSetupParamSet_t *TaskSetupParams)
{
   ...
   TaskId TaskNum = (TaskId)SDMA_TASKNUM_EXT(TaskAPI->TaskNum);
#if (DEBUG_BESTCOMM_API>0) || (MAX_BD>0)
   uint32 Status  = INCR_SRC & ~SDMA_BD_MASK_READY; /* Tx:Status=0, Rx:Status!=0 && ready=0 */
   uint8  NumPtr  = 0;
#endif
   ...
   if (Status!=0) printf("TaskSetup: Rx task\n");
