...
read_dcr(int n)
{
    ...
    instrs[0] = 0x7c600286 + ((n & 0x1F) << 16) + ((n & 0x3e0) << 6);
    instrs[1] = 0x4e800020;
    store_inst(instrs);
    store_inst(instrs+1);
    code = (int (*)(void)) instrs;
...
void
write_dcr(int n, unsigned int val)
{
    unsigned int instrs[2];
    int (*code)(unsigned int);

    instrs[0] = 0x7c600386 + ((n & 0x1F) << 16) + ((n & 0x3e0) << 6);
    instrs[1] = 0x4e800020;
    store_inst(instrs);
    store_inst(instrs+1);
    code = (int (*)(unsigned int)) instrs;
...
device_regs(void)
{
	...
	case 'w':
		...
		/* fall through */
	case 'r':
		printf("dcr %x = %x\n", regno, read_dcr(regno));
		break;
	case 'i':
		scanhex(&val);
		write_dcr(regno, val);
		printf("dcr %x(%x) = %x\n", regno, val, read_dcr(regno+1));
		break;
	}
	scannl();
}
