...
#undef  writel
#define writel(data,addr)	out_be32((addr),(data))

#define ohci_le16_to_cpu(x)	(x)
#define ohci_cpu_to_le16(x)	(x)
#define ohci_cpu_to_le32(x)	(x)
#define ohci_le32_to_cpup(x)	(*(x))

#else

#define ohci_le16_to_cpu(x)	le16_to_cpu(x)
#define ohci_cpu_to_le16(x)	cpu_to_le16(x)
#define ohci_cpu_to_le32(x)	cpu_to_le32(x)
#define ohci_le32_to_cpup(x)	le32_to_cpup(x)
...
void ep_print_int_eds (ohci_t * ohci, char * str) {
	...
	for (i= 0; i < 32; i++) {
		...
		while (*ed_p != 0 && j--) {
			ed_t *ed = dma_to_ed (ohci, ohci_le32_to_cpup(ed_p));
		...
		case PIPE_ISOCHRONOUS:
			if (urb->transfer_flags & USB_ISO_ASAP) { 
				urb->start_frame = ((ed->state == ED_OPER)
					? (ed->last_iso + 1)
					: (ohci_le16_to_cpu (ohci->hcca->frame_no) + 10)) & 0xffff;
			}	
			/* FALLTHROUGH */
		case PIPE_INTERRUPT:
			if (urb->bandwidth == 0) {
	...
	if (usb_dev->devnum >= 0) {
		...
		if (cnt > 0) {
			...
			if (ohci->disabled) {
				 ...
				 * these lists again!!
				dl_del_list (ohci,
					ohci_le16_to_cpu (ohci->hcca->frame_no) & 1);
				 */
				warn ("TD leak, %d", cnt);
...
static int sohci_get_current_frame_number (struct usb_device *usb_dev) 
{
	...
	return ohci_le16_to_cpu (ohci->hcca->frame_no);
}
...
{	 
	...
	case PIPE_INTERRUPT:
		...
		for (i = 0; i < ep_rev (6, interval); i += inter) {
			...
			for (ed_p = &(ohci->hcca->int_table[ep_rev (5, i) + int_branch]); 
				(*ed_p != 0) && ((dma_to_ed (ohci, ohci_le32_to_cpup (ed_p)))->int_interval >= interval); 
				ed_p = &((dma_to_ed (ohci, ohci_le32_to_cpup (ed_p)))->hwNextED)) 
					inter = ep_rev (6, (dma_to_ed (ohci, ohci_le32_to_cpup (ed_p)))->int_interval);
			ed->hwNextED = *ed_p; 
			*ed_p = ohci_cpu_to_le32 (ed->dma);
		}
#ifdef DEBUG
	...
	case PIPE_ISOCHRONOUS:
		...
		if (ohci->ed_isotail != NULL) {
			ohci->ed_isotail->hwNextED = ohci_cpu_to_le32 (ed->dma);
...
static void periodic_unlink (
	...
	for (; index < NUM_INTS; index += period) {
		...
		while (*ed_p != 0) {
			if ((dma_to_ed (ohci, ohci_le32_to_cpup (ed_p))) == ed) {
				*ed_p = ed->hwNextED;		
				break;
			}
			ed_p = & ((dma_to_ed (ohci,
				ohci_le32_to_cpup (ed_p)))->hwNextED);
		}
	}	
}
...
static int ep_unlink (ohci_t * ohci, ed_t * ed) 
{
	...
	case PIPE_CONTROL:
		if (ed->ed_prev == NULL) {
			...
			writel (ohci_le32_to_cpup (&ed->hwNextED), &ohci->regs->ed_controlhead);
		} else {
		...
		if (ohci->ed_controltail == ed) {
			...
			(dma_to_ed (ohci, ohci_le32_to_cpup (&ed->hwNextED)))->ed_prev = ed->ed_prev;
		}
		break;
	...
	case PIPE_BULK:
		if (ed->ed_prev == NULL) {
			...
			writel (ohci_le32_to_cpup (&ed->hwNextED), &ohci->regs->ed_bulkhead);
		} else {
			ed->ed_prev->hwNextED = ed->hwNextED;
		}
	...
	case PIPE_ISOCHRONOUS:
		...
		if (ed->hwNextED != 0) 
		    (dma_to_ed (ohci, ohci_le32_to_cpup (&ed->hwNextED)))
		    	->ed_prev = ed->ed_prev;
...
{    
	...
	frame = ohci_le16_to_cpu (ohci->hcca->frame_no) & 0x1;
	...
	struct urb * urb, int index)
{
	...
	td = urb_priv->td [index] = dma_to_td (ohci,
			ohci_le32_to_cpup (&urb_priv->ed->hwTailP) & ~0xf);
...
static void dl_transfer_length(td_t * td)
{
	...
	tdINFO = ohci_le32_to_cpup (&td->hwINFO);
  	tdBE   = ohci_le32_to_cpup (&td->hwBE);
  	tdCBP  = ohci_le32_to_cpup (&td->hwCBP);


  	if (tdINFO & TD_ISO) {
 		tdPSW = ohci_le16_to_cpu (td->hwPSW[0]);
 		cc = (tdPSW >> 12) & 0xF;
		if (cc < 0xE)  {
...
static td_t * dl_reverse_done_list (ohci_t * ohci)
{
	...
	td_list_hc = ohci_le32_to_cpup (&ohci->hcca->done_head) & 0xfffffff0;
	ohci->hcca->done_head = 0;
	
	while (td_list_hc) {		
		td_list = dma_to_td (ohci, td_list_hc);

		if (TD_CC_GET (ohci_le32_to_cpup (&td_list->hwINFO))) {
			urb_priv = (urb_priv_t *) td_list->urb->hcpriv;
			dbg(" USB-error/status: %x : %p", 
					TD_CC_GET (ohci_le32_to_cpup (&td_list->hwINFO)), td_list);
			if (td_list->ed->hwHeadP & ohci_cpu_to_le32 (0x1)) {
				if (urb_priv && ((td_list->index + 1) < urb_priv->length)) {
					td_list->ed->hwHeadP = 
						(urb_priv->td[urb_priv->length - 1]->hwNextTD & ohci_cpu_to_le32 (0xfffffff0)) |
		...
		td_rev = td_list;
		td_list_hc = ohci_le32_to_cpup (&td_list->hwNextTD) & 0xfffffff0;	
	}	
	spin_unlock_irqrestore (&usb_ed_lock, flags);
	return td_list;
}
...
static void dl_del_list (ohci_t  * ohci, unsigned int frame)
{
	...
	for (ed = ohci->ed_rm_list[frame]; ed != NULL; ed = ed->ed_rm_list) {
		...
		tdTailP = dma_to_td (ohci, ohci_le32_to_cpup (&ed->hwTailP) & 0xfffffff0);
		tdHeadP = dma_to_td (ohci, ohci_le32_to_cpup (&ed->hwHeadP) & 0xfffffff0);
		edINFO = ohci_le32_to_cpup (&ed->hwINFO);
		td_p = &ed->hwHeadP;
		...
		for (td = tdHeadP; td != tdTailP; td = td_next) { 
			...
			td_next = dma_to_td (ohci, ohci_le32_to_cpup (&td->hwNextTD) & 0xfffffff0);
			if ((urb_priv->state == URB_DEL) || (ed->state & ED_DEL)) {
				tdINFO = ohci_le32_to_cpup (&td->hwINFO);
				if (TD_CC_GET (tdINFO) < 0xE)
					dl_transfer_length (td);
				*td_p = td->hwNextTD | (*td_p & ohci_cpu_to_le32 (0x3));
	...
	urb_priv_t * urb_priv;
 	...
 	unsigned long flags;
  	...
  	while (td_list) {
  		...
  		urb = td_list->urb;
  		urb_priv = urb->hcpriv;
  		tdINFO = ohci_le32_to_cpup (&td_list->hwINFO);
  		...
  		spin_lock_irqsave (&usb_ed_lock, flags);
  		if (ed->state != ED_NEW) { 
  			edHeadP = ohci_le32_to_cpup (&ed->hwHeadP) & 0xfffffff0;
  			edTailP = ohci_le32_to_cpup (&ed->hwTailP);
...
static void hc_interrupt (int irq, void * __ohci, struct pt_regs * r)
{
	...
	if ((ohci->hcca->done_head != 0)
			&& !(ohci_le32_to_cpup (&ohci->hcca->done_head) & 0x01)) {
		ints =  OHCI_INTR_WDH;
	...
	// dbg("Interrupt: %x frame: %x", ints, ohci_le16_to_cpu (ohci->hcca->frame_no));
	...
	if (ints & OHCI_INTR_SF) { 
		unsigned int frame = ohci_le16_to_cpu (ohci->hcca->frame_no) & 1;
...
static struct pci_dev dummy_ohci_dev =
{
	.name		= "Built-In ohci",
	.slot_name	= "0",
...
static struct pci_device_id dummy_ohci_id =
{
	.driver_data	= 0
};
...
static int __init mpc5200_ohci_init (void) 
{
	int ret;
	struct mpc5xxx_cdm	*cdm = (struct mpc5xxx_cdm *)MPC5xxx_CDM;
	struct mpc5xxx_gpio *gpio = (struct mpc5xxx_gpio *)MPC5xxx_GPIO;
	u32 port_config = in_be32(&gpio->port_config);

	/* Enable clocks */

	// XX HACK ALERT
	//
	// Assume 33Mhz input clock & FVCO = INPUTx16 or
	// FVCO = INPUTx12 only for now
	 ...
	 */
	ret = hc_found_ohci(&dummy_ohci_dev, MPC5xxx_USB_IRQ, (void *)MPC5xxx_USB_OHCI,
			    &dummy_ohci_id);
...
static void __exit mpc5200_ohci_cleanup (void) 
{	
	ohci_pci_remove (&dummy_ohci_dev);
}

module_init (mpc5200_ohci_init);
module_exit (mpc5200_ohci_cleanup);
