/*
 * Ethernet driver for the ISP1362 USB client function.
...
usb_recv_callback(int flag, int size)
{
	...
	isp1362_recv(cur_rx_skb->tail, MIN(usb_rsize, skb_tailroom (cur_rx_skb)),
			usb_recv_callback);
	if (!skb)
		return;
...
//		if ( flag == -EINTR || flag == -EAGAIN ) // only if we are coming out of stall
			isp1362_recv(cur_rx_skb->tail, MIN(usb_rsize, skb_tailroom (cur_rx_skb)), usb_recv_callback);
	}
}
...
usb_send_callback(int flag, int size)
{
	...
	ret = isp1362_send(cur_tx_skb->data, cur_tx_skb->len, usb_send_callback);
	if (ret) {
...
usb_eth_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (cur_tx_skb) {
		...
		cur_tx_skb = skb;
		dev->trans_start = jiffies;
		ret = isp1362_send(skb->data, skb->len, usb_send_callback);
		if (ret) {
			/* If the USB core can't accept the packet, we drop it. */
			dev_kfree_skb(skb);
			cur_tx_skb = NULL;
...
MODULE_PARM_DESC(usb_rsize, "number of bytes in packets from host to isp1362");
MODULE_PARM(usb_wsize, "1i");
MODULE_PARM_DESC(usb_wsize, "number of bytes in packets from isp1362 to host");
MODULE_LICENSE("GPL");
#endif
...
usb_eth_init(void)
{
	...
	if ( rc == 0 ) {
		 ...
		 pd->dev.idVendor	  = make_word_c(ETHERNET_VENDOR_ID);
		 pd->dev.idProduct	  = make_word_c(ETHERNET_PRODUCT_ID);
		 pstr = isp1362_kmalloc_string_descriptor( "ISP1362 USB NIC" );
		 if ( pstr ) {
			  isp1362_set_string_descriptor( 1, pstr );
			  pd->dev.iProduct = 1;
		 }
		 rc = isp1362_start();
	}
...
usb_eth_cleanup(void)
{
	...
	isp1362_stop();
	isp1362_close();
	if ( (pstr = isp1362_get_string_descriptor(1)) != NULL )
		kfree( pstr );
