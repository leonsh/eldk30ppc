...
static int pm_con_request(struct pm_dev *dev, pm_request_t rqst, void *data)
{
	...
	case PM_RESUME:
		...
		break;
	case PM_SUSPEND:
		/* We temporarily set vesa_off_interval to prevent
		 * firing up the console timer during suspend
		 */
		save_vesa_interval = vesa_off_interval;
		vesa_off_interval = 0;
		do_blank_screen(0);
		vesa_off_interval = save_vesa_interval;
		break;
	}
	return 0;
}
