...
cpm_iic_write(struct i2c_algo_8xx_data *cpm, u_char abyte, char *buf,int count)
{
	...
	*tb = abyte;		/* Device address byte w/rw flag */
	...
	if ((tbdf[0].cbd_sc | tbdf[1].cbd_sc) & BD_SC_READY) {
		if (cpm_debug)
			printk(KERN_DEBUG "IIC write; complete but tbuf ready\n");
		return 0;
	}
		    ...
		    int num)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (pmsg->flags & I2C_M_RD ) {
			...
			if (cpm_debug)
				printk(KERN_DEBUG "i2c-algo-8xx.o: read %d bytes\n", ret);
			if (ret < pmsg->len ) {
...
int i2c_8xx_add_bus(struct i2c_adapter *adap)
{
	...
	if (cpm_debug)
		printk(KERN_DEBUG "i2c-algo-8xx.o: hw routines for %s registered.\n",
		       adap->name);
...
int i2c_8xx_del_bus(struct i2c_adapter *adap)
{
	...
	printk(KERN_INFO "i2c-algo-8xx.o: adapter unregistered: %s\n",adap->name);
