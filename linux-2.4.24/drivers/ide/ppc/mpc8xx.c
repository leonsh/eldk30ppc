 ...
 * which must be correctly initialized by the firmware (PPCBoot/U-Boot).
 */
		...
		ide_ioreg_t ctrl_port, int *irq)
{
	...
	*p = 0;
	...
	if (check_ide_device (pcmcia_base))
		return;

#endif	/* CONFIG_IDE_8xx_PCCARD */

	base = pcmcia_base + ioport_dsc[data_port].base_off - _IO_BASE;
#ifdef DEBUG
	printk ("base: %08lx + %08lx = %08lx\n",
			pcmcia_base, ioport_dsc[data_port].base_off, base);
#endif

	for (i = 0; i < IDE_NR_PORTS; ++i) {
#ifdef DEBUG
		printk ("port[%d]: %08lx + %08x = %08lx\n",
			i,
			base,
