...
#ifdef CONFIG_AMX860
	/* Port B is used to PHY control,MC68160.
	*/
	immap->im_cpm.cp_pbdir |= 
	        (PB_ENET_ETHLOOP | PB_ENET_TPFLDL | PB_ENET_TPSQEL);
	immap->im_cpm.cp_pbdat |= PB_ENET_TPFLDL;
	immap->im_cpm.cp_pbdat &= ~(PB_ENET_ETHLOOP | PB_ENET_TPSQEL);

	immap->im_ioport.iop_pddir |= PD_ENET_ETH_EN;
	immap->im_ioport.iop_pddat &= ~PD_ENET_ETH_EN;
#endif
