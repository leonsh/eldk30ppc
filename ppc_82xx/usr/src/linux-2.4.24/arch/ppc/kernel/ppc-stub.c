...
handle_exception (struct pt_regs *regs)
{
...
#ifdef KGDB_DEBUG
	printk(KERN_DEBUG "kgdb: entering handle_exception; trap [0x%x]\n",
	...
	*ptr++ = 0;
	...
	if (kdebug)
		printk(KERN_DEBUG "remcomOutBuffer: %s\n", remcomOutBuffer);
 ...
 */
			...
			kgdb_active = 0;
			if (kdebug) {
				printk(KERN_DEBUG "remcomInBuffer: %s\n",
						remcomInBuffer);
				printk(KERN_DEBUG "remcomOutBuffer: %s\n",
						remcomOutBuffer);
			}
...
{
	if (!initialized) {
		printk(KERN_ERR "breakpoint() called before kgdb init\n");
		return;
	}
