 ...
 * Critical Linux, Xyterra, Zuma Networks, and MontaVista Software were
 * heavily involved.
 */
 ...
 *
 * There is now a 64260B-B-0 but I have not yet had one to test so I don't
 * know if it will work--MAG.
 */
...
static void __init gt64260_setup_mem_windows(gt64260_bridge_info_t *info);

/*
 * Typical '_find_bridges()' routine for boards with a GT64260 bridge.
 *
 * Assumes that gt64260_base the virtual base address of the bridge's registers.
 */
int __init
gt64260_find_bridges(gt64260_bridge_info_t *info,
	int ((*map_irq)(struct pci_dev *, unsigned char, unsigned char)))
{
	struct pci_controller		*hose_a, *hose_b;
	u32				io_base_a, io_base_b, val;
	int				rc = 0;
	...
	if ((hose_a == (struct pci_controller *)NULL) &&
	    (hose_b == (struct pci_controller *)NULL)) {

		if (ppc_md.progress)
			ppc_md.progress("gt64260_find_bridges: no hoses", 0x0);

		printk("gt64260_find_bridges: No PCI hoses\n");
		return -1;
	}

	/*
	 * Map in PCI I/O region.
	 * ioremap() typically grows down.  Try to put io_base_a lower
	 * than io_base_b so allocate hose_b's PCI io space first.
	 */
	if (hose_b != (struct pci_controller *)NULL) {
		io_base_b = (u32)ioremap(info->pci_1_io_start_proc,
					info->pci_1_io_size);
		isa_io_base = io_base_b;
	}

	/*
	 * We need to scan the PCI bus before doing the bridge init because
	 * we need the buses enumerated.  This is okay because the only regs
	 * that are accessed during the scanning are the PCI config addr/data
	 * pair on the bridge and config registers on PCI devices (and the
	 * bridge's regs on the PCI bus will be excluded).
	 */

	/*
	 * Set up the PCI resources/hose structure and the scan the PCI buses
	 * under the first PCI hose.
	 */
	if (hose_a != (struct pci_controller *)NULL) {
		io_base_a = (u32)ioremap(info->pci_0_io_start_proc,
					info->pci_0_io_size);
		isa_io_base = io_base_a;

		setup_indirect_pci(hose_a,
			   info->phys_base_addr + GT64260_PCI_0_CONFIG_ADDR,
			   info->phys_base_addr + GT64260_PCI_0_CONFIG_DATA);

		pci_init_resource(&hose_a->io_resource,
			  io_base_a - isa_io_base,
			  io_base_a - isa_io_base + info->pci_0_io_size - 1,
			  IORESOURCE_IO,
		...
		/* Can't walk PCI bus until bridge is initialized */
	}

	/* Now scan the second PCI hose */
	if (hose_b != (struct pci_controller *)NULL) {
		setup_indirect_pci(hose_b,
			   info->phys_base_addr + GT64260_PCI_1_CONFIG_ADDR,
			   info->phys_base_addr + GT64260_PCI_1_CONFIG_DATA);
	...
	/* Now we can initialize the bridge for real */
	...
	if (hose_a != (struct pci_controller *)NULL) {
		...
		hose_a->last_busno = pciauto_bus_scan(hose_a,
							hose_a->first_busno);
	}
	...
	if (hose_b != (struct pci_controller *)NULL) {
		...
		if (hose_a != (struct pci_controller *)NULL) {
			...
			/* Set bus number for second hose */
			val = gt_read(GT64260_PCI_1_P2P_CONFIG);
			val &= 0xff00ffff;
			val |= ((hose_b->first_busno & 0xff) << 16);
			gt_write(GT64260_PCI_1_P2P_CONFIG, val);
			(void)gt_read(GT64260_PCI_1_P2P_CONFIG);/* Flush FIFO */
		}
	...
	/*
	 * Set up the CPU memory-related windows and the PCI BARs based on how
	 * the CPU SCS windows are set up.  Although, not clearly documented,
	 * testing has shown that you need the PCI SCS windows to match the
	 * CPU SCS windows.
	 */
	gt64260_setup_mem_windows(info);

	/* Check for errata and adjust accordingly */
...
gt64260_bridge_init(gt64260_bridge_info_t *info)
{
	...
	/* Bit 12 MUST be 0. */
	gt_clr_bits(GT64260_CPU_CONFIG, (1<<12));
	 ...
	 * or disabling.
	 */
	gt_clr_bits(GT64260_TIMR_CNTR_0_3_CNTL,
			((1<<0) | (1<<8) | (1<<16) | (1<<24)));
	gt_clr_bits(GT64260_TIMR_CNTR_4_7_CNTL,
			((1<<0) | (1<<8) | (1<<16) | (1<<24)));

	/*
	 * Shut down CPU windows and PCI BAR's so there aren't any conflicts
	 * with the new windows we're about to set up.
	 *
	 * Note: Can't disable CPU SCS, snoop, and protection windows because
	 * we're [probably] running out of one of those windows.
	 */
	gt64260_cpu_disable_windows();
	gt64260_pci_bar_enable(0, 0); /* Disable BARs on PCI side of bridge */
	gt64260_pci_bar_enable(1, 0);
	 ...
	 * be set up.
	 */
	if (info->hose_a != (struct pci_controller *)NULL) {
		...
		u32_val = (info->pci_0_latency<<8) | (L1_CACHE_LINE_SIZE>>2);
		early_write_config_dword(info->hose_a,
					 0,
					 PCI_DEVFN(0,0),
					 PCI_CACHE_LINE_SIZE,
		...
		/* Set windows from CPU to PCI I/O and MEM space */
	...
	if (info->hose_b != (struct pci_controller *)NULL) {
		save_exclude = gt64260_pci_exclude_bridge;
		gt64260_pci_exclude_bridge = FALSE;
		...
		/* Set latency timer, cache line size, clear BIST */
		u32_val = (info->pci_1_latency<<8) | (L1_CACHE_LINE_SIZE>>2);
		early_write_config_dword(info->hose_b,
					 0,
					 PCI_DEVFN(0,0),
					 PCI_CACHE_LINE_SIZE,
...
/*
 * gt64260_setup_mem_windows()
 *
 * Set up all the CPU and PCI windows to match how the CPU SCS size windows
 * are set up.  Although this doesn't appear to be necessary according to the
 * manual, it appears to be necessary from experience with the chip.
 *
 * This routine is the result of a patch from Uri Lublin <uri@sangate.com>.
 */
static void __init
gt64260_setup_mem_windows(gt64260_bridge_info_t *info)
{
	u32	low, hi, size, window, enable_bits;
	const struct {
		int low,hi;
	} scs_offset_table[GT64260_CPU_SCS_DECODE_WINDOWS] = {
...
#define SCS_ADDR(a) (((a) & 0x0fff) << 20)
	enable_bits = 0;

	for (window=0; window<GT64260_CPU_SCS_DECODE_WINDOWS; window++) {
		low = gt_read(scs_offset_table[window].low);
		hi  = gt_read(scs_offset_table[window].hi);

		low = SCS_ADDR(low);
		hi = SCS_ADDR(hi);

		size = (low <= hi) ? (hi - low + (SCS_ADDR(1))) : 0;
		if (size) {
			enable_bits |= (1 << window);

#ifdef DEBUG
			printk(__FUNCTION__ ": scs window: %d, %x (%x)\n",
					window, low, size);
#endif
			gt64260_cpu_prot_set_window(window, low, size,
					info->cpu_prot_options[window]);
			gt64260_cpu_snoop_set_window(window, low, size,
					     info->cpu_snoop_options[window]);

			if (info->hose_a != (struct pci_controller *)NULL) {
				gt64260_pci_slave_scs_set_window(info->hose_a,
						window, low, low, size);
				gt64260_pci_acc_cntl_set_window(0, window, 0,
					low, size,
					info->pci_0_acc_cntl_options[window]); 
				gt64260_pci_snoop_set_window(0, window, 0,
					low, size,
					info->pci_0_snoop_options[window]); 
			}
			if (info->hose_b != (struct pci_controller *)NULL) {
				gt64260_pci_slave_scs_set_window(info->hose_b,
						window, low, low, size);
				gt64260_pci_acc_cntl_set_window(1, window, 0,
					low, size,
					info->pci_1_acc_cntl_options[window]); 
				gt64260_pci_snoop_set_window(1, window, 0,
					low, size,
					info->pci_1_snoop_options[window]); 
			}
		}
	}

	if (info->hose_a != (struct pci_controller *)NULL) {
		gt64260_pci_bar_enable(0, enable_bits);
	}

	if (info->hose_b != (struct pci_controller *)NULL) {
		gt64260_pci_bar_enable(1, enable_bits);
	}
}
	...
	if (gt64260_revision == GT64260) {
		...
		if (hose_a != (struct pci_controller *)NULL) {
			...
			early_write_config_dword(hose_a,
						 ...
						 val);
			/* FEr#12, FEr#13 */
			gt_clr_bits(GT64260_PCI_0_CMD,
					((1<<4) | (1<<5) | (1<<9)));
			/* FEr#12, FEr#13 */
			gt_clr_bits(GT64260_PCI_0_CMD,
					((1<<4) | (1<<5) | (1<<9)));

			gt_clr_bits(GT64260_PCI_0_CMD, (1<<13));
		}
		...
		if (hose_b != (struct pci_controller *)NULL) {
			...
			early_write_config_dword(hose_b,
						 ...
						 val);
			/* FEr#12, FEr#13 */
			gt_clr_bits(GT64260_PCI_1_CMD,
					((1<<4) | (1<<5) | (1<<9)));
			/* FEr#12, FEr#13 */
			gt_clr_bits(GT64260_PCI_1_CMD,
					((1<<4) | (1<<5) | (1<<9)));
			gt_clr_bits(GT64260_PCI_1_CMD, (1<<13));
		}
		 ...
		 */
		if (hose_a != (struct pci_controller *)NULL) {
			gt_set_bits(GT64260_PCI_0_CMD,
				((1<<4) | (1<<5) | (1<<9) | (1<<13)));
		}

		if (hose_b != (struct pci_controller *)NULL) {
			gt_set_bits(GT64260_PCI_1_CMD,
				((1<<4) | (1<<5) | (1<<9) | (1<<13)));
		}
	}
...
void
gt64260_cpu_disable_windows(void)
{
	int	pci_bus, window;

	/* Leave SCS windows alone because we're running from memory */

	/*
	 * Note: Don't put any progress msgs between shutting down CS windows
	 * and re-enabling them because the console device may be behind one
	 * of those windows.
	 */
	for (window=0; window<GT64260_PCI_CS_WINDOWS; window++) {
...
#ifdef DEBUG
	printk(__FUNCTION__ ": setting busno %08x, devfn %08x, offset %08x -> %08x\n",
		hose->first_busno,  devfn, pci_cfg_hdr_offset, pci_base_addr | val);
#endif
...
#ifdef DEBUG
		printk(__FUNCTION__ ": bus %d: hose->first_busno: %d\n", pci_bus, hose->first_busno);
#endif
...
{
	static u32
	pci_boot_table[GT64260_PCI_BUSES][2] = {
		{ /* PCI 0 */
		  GT64260_PCI_0_SLAVE_BOOT_SIZE,
		  GT64260_PCI_0_SLAVE_BOOT_REMAP,
		},
		{ /* PCI 1 */
	...
	rc = gt64260_pci_slave_set_window(hose,
					  ...
					  1,
					  0x20,
					  pci_boot_table[pci_bus][0],
					  pci_boot_table[pci_bus][1]);
...
{
	static u32
	pci_p2p_io_table[GT64260_PCI_BUSES][2] = {
		{ /* PCI 0 */
		  GT64260_PCI_0_SLAVE_P2P_IO_SIZE,
		  GT64260_PCI_0_SLAVE_P2P_IO_REMAP,
		},
		{ /* PCI 1 */
	...
	rc = gt64260_pci_slave_set_window(hose,
				  ...
				  2,
				  0x18,
				  pci_p2p_io_table[pci_bus][0],
				  pci_p2p_io_table[pci_bus][1]);
...
{
	static u32
	pci_dac_boot_table[GT64260_PCI_BUSES][2] = {
		{ /* PCI 0 */
		  GT64260_PCI_0_SLAVE_DAC_BOOT_SIZE,
		  GT64260_PCI_0_SLAVE_DAC_BOOT_REMAP,
		},
		{ /* PCI 1 */
		  GT64260_PCI_1_SLAVE_DAC_BOOT_SIZE,
		  GT64260_PCI_1_SLAVE_DAC_BOOT_REMAP,
		}
	}; /* pci_dac_boot_table[][] */
	int	pci_bus;
	int	rc;
	...
	rc = gt64260_pci_slave_set_window(hose,
					  ...
					  7,
					  0x18,
					  pci_dac_boot_table[pci_bus][0],
					  pci_dac_boot_table[pci_bus][1]);
 ...
 */
int
gt64260_set_base(u32 new_phys_base)
{
	u32	val;
	int	limit = 100000;
	int	rc = 0;
	...
	val = (new_phys_base >> 20) | (val & 0xffff0000);
	gt_write(GT64260_INTERNAL_SPACE_DECODE, val);

	iounmap((void *)gt64260_base);
	gt64260_base = (u32)ioremap((new_phys_base & 0xfff00000),
				    GT64260_INTERNAL_SPACE_SIZE);

	/* Wait for bridge to move its regs */
	while ((gt_read(GT64260_INTERNAL_SPACE_DECODE)!=val) && (limit-- > 0));
 ...
 *
 * Read memory controller's registers to determine the amount of memory
 * in the system.  Assumes that the memeory controller has been set up
 * to the proper memory size.
 */
ulong __init
gt64260_get_mem_size(void)
{
	static u32
	cpu_scs_windows[GT64260_CPU_SCS_DECODE_WINDOWS][2] = {
		{ GT64260_CPU_SCS_DECODE_0_BOT, GT64260_CPU_SCS_DECODE_0_TOP },
	...
	if (first_time) {
		for (window=0; window<GT64260_CPU_SCS_DECODE_WINDOWS; window++){
			bot = gt_read(cpu_scs_windows[window][0]) << 20;
			top = (gt_read(cpu_scs_windows[window][1]) << 20) +
								0x00100000;

			if (top > bot) { /* Is window really open? */
				total += (top - bot);
			}
		}
