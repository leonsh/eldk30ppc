...
#if defined(CONFIG_WATCHDOG) &&  defined(CONFIG_8260)
extern void wdt_mpc8xx_reset (void);
# define WATCHDOG_RESET wdt_mpc8xx_reset()
#else
...
/* LWMON has a nasty 100 ms watchdog that needs special "care" */
#if defined(CONFIG_WATCHDOG) && defined(CONFIG_8xx) 
	extern int wdt_mpc8xx_is_ready;
	/*
	 * Trigger watchdog until watchdog driver has initialized,
	 * but never longer than 5 seconds
	 */
	if ((wdt_mpc8xx_is_ready == 0) &&
	    (jiffies % (HZ / 20) == 0) &&
	    (jiffies < 5 * HZ) ) {
		wdt_mpc8xx_reset();
	}
...
#if defined(CONFIG_RTC_11_MINUTE_MODE)
		/*
...
#endif /* CONFIG_RTC_11_MINUTE_MODE */
