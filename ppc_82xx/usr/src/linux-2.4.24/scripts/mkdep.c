...
static void
handle_symlink(const char *filename)
{
	struct stat st;
	...
	if (S_ISLNK(st.st_mode)) {
		...
		/* mv filename filename.lnk */
		sprintf(lnk, "%s.lnk", filename);

		if (link(filename, lnk) || unlink(filename)) {
			free (lnk);
		...
		/* copy filename.lnk to filename */
		ifd = open(lnk, O_RDONLY);
		if (ifd == -1) {
			perror("open");
			link(lnk, filename);
			goto cleanup;
		...
		buf = malloc(bufsz);
		if (!buf) {
			perror("out of memory for buf");
			unlink(filename);
			link(lnk, filename);
			goto cleanup;
		}
					
		while ((cnt = read(ifd, buf, bufsz)) > 0) {
			if (write(ofd, buf, cnt) < cnt) {
				perror("short write");
				unlink(filename);
				link(lnk, filename);
				break; /* fall to cleanup */
			}
		}
