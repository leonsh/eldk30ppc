...
void tqm8xxl_copy_from(struct map_info *map,
...
void tqm8xxl_copy_to(struct map_info *map,
...
/*	size:	MTDPART_SIZ_FULL, */
    ...
    /* overlapping second flash bank with one big filesystem */
    {
	name:	"big_fs",
	offset:	0x00400000,
	size:	0x00400000,
...
int __init init_tqm_mtd(void)
{
	...
	for(idx = 0 ; idx < FLASH_BANK_MAX ; idx++) {
		...
		sprintf(map_banks[idx]->name, "TQM8xxL Bank %d", idx);
	...
	if (!num_banks) {
		printk("TQM8xxL: No supported flash chips found!\n");
		ret = -ENXIO;
		goto error_mem;
	}
