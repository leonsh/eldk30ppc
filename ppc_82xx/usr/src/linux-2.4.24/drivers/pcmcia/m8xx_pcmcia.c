...
/* The TQM8xx series use SLOT_B */

#if defined(CONFIG_TQM8xxL)
#ifndef CONFIG_NSCU
#define CONFIG_PCMCIA_SLOT_B

/* The NSCU uses 2 slots */

#else
#define NSCU_OE_INV 1
#define CONFIG_PCMCIA_SLOT_A
#define CONFIG_PCMCIA_SLOT_B
#endif
#endif

/* The C2MON uses SLOT_B */

#ifdef CONFIG_C2MON
#define CONFIG_PCMCIA_SLOT_B
#endif

/* The ICU862 uses SLOT_B */
...
#if defined(CONFIG_RPXCLASSIC) || defined(CONFIG_RPXLITE) || defined(CONFIG_RMU)
#define CONFIG_PCMCIA_SLOT_B
#endif
...
/* ------------------------------------------------------------------------- */
/* TQM8xxL/M Boards by TQ Components                                           */

#if defined(CONFIG_TQM8xxL)

# if defined(CONFIG_TQM8xxM)
#  define PCMCIA_BOARD_MSG "TQM8xxM"
# else
...
#define __MY_PCMCIA_GCRX_CXRESET	M8XX_PGCRX_CXOE
#define __MY_PCMCIA_GCRX_CXOE		M8XX_PGCRX_CXRESET
#else
#define __MY_PCMCIA_GCRX_CXRESET	M8XX_PGCRX_CXRESET
#define __MY_PCMCIA_GCRX_CXOE		M8XX_PGCRX_CXOE
#endif
...
static int hardware_enable(int slot)
{
	...
	DEBUG (3,"hardware_enable: " PCMCIA_BOARD_MSG " Slot %c\n", 'A'+slot);

	udelay(10000);

	immap = (immap_t *)IMAP_ADDR;
	sysp  = (sysconf8xx_t *)(&(((immap_t *)IMAP_ADDR)->im_siu_conf));
	...
	pcmp->pcmc_pscr =  M8XX_PCMCIA_MASK(slot);
	pcmp->pcmc_per &= ~M8XX_PCMCIA_MASK(slot);

	/*
	 * Disable PCMCIA buffers (isolate the interface)
	 * and assert RESET signal
	 */
	DEBUG (3,"Disable PCMCIA buffers and assert RESET\n");
	reg  =  M8XX_PGCRX(slot);
	 ...
	 */
	udelay(10000);
	DEBUG (3,"[%d] %s: PIPR(%p)=0x%x\n",
		__LINE__,__FUNCTION__,
		&(pcmp->pcmc_pipr),pcmp->pcmc_pipr);
	if (pcmp->pcmc_pipr & 0x00001800) {
		DEBUG (3,"   No Card found\n");
		return (1);
	}
	 ...
	 * Power On.
	 */
	mask = M8XX_PCMCIA_VS1(slot) | M8XX_PCMCIA_VS2(slot);
	reg  = pcmp->pcmc_pipr;
	DEBUG (3,"PIPR: 0x%x ==> VS1=o%s, VS2=o%s\n",
		reg,
		(reg&M8XX_PCMCIA_VS1(slot))?"n":"ff",
		(reg&M8XX_PCMCIA_VS2(slot))?"n":"ff");
#ifndef CONFIG_NSCU
	if ((reg & mask) == mask) {
		immap->im_ioport.iop_pcdat |= 0x0004;
		DEBUG (3," 5.0V card found: ");
	} else {
		immap->im_ioport.iop_pcdat |= 0x0002;
		DEBUG (3," 3.3V card found: ");
	}
	...
	DEBUG (3,"Enable PCMCIA buffers and stop RESET\n");
	reg  =  M8XX_PGCRX(slot);
	reg &= ~__MY_PCMCIA_GCRX_CXRESET;	/* active high */
#ifdef NSCU_OE_INV
	reg |= __MY_PCMCIA_GCRX_CXOE;		/* active low  */
...
static int hardware_disable(int slot)
{
	...
	DEBUG (3,"hardware_disable: " PCMCIA_BOARD_MSG " Slot %c\n", 'A'+slot);
...
static int voltage_set(int slot, int vcc, int vpp)
{
...
#endif
	if (reg) {
		DEBUG (3,"PCMCIA powered at %sV\n",
			(reg&0x0004) ? "5.0" : "3.3");
	} else {
		DEBUG (3,"PCMCIA powered down\n");
	}
	...
	DEBUG (3,"voltage_set: " PCMCIA_BOARD_MSG " Slot %c, DONE\n",
		slot+'A');
	return (0);
}
...
#define SIUMCR_DBGC11	0x00600000	/* Debug pins configuration */
...
static int hardware_enable(int slot)
{
	...
	/* clear interrupt state, and disable interrupts */
	pcmp->pcmc_pscr =  M8XX_PCMCIA_MASK(_slot_);
	pcmp->pcmc_per &= ~M8XX_PCMCIA_MASK(_slot_);
	 ...
	 */
	DEBUG (3, "Disable PCMCIA buffers and assert RESET\n");
	reg  =  M8XX_PGCRX(_slot_);
	reg |=  M8XX_PGCRX_CXRESET;	/* active high */
	reg |=  M8XX_PGCRX_CXOE;		/* active low  */
	M8XX_PGCRX(_slot_) = reg;
	udelay(500);
	...
	DEBUG (3, "PIPR: 0x%x ==> VS1=o%s, VS2=o%s\n",
		pipr,
		(pipr&M8XX_PCMCIA_VS1(slot))?"n":"ff",
		(pipr&M8XX_PCMCIA_VS2(slot))?"n":"ff");
	...
	if ((pipr & mask) == mask) {
		...
		DEBUG (0," 5.0V card found: ");
	} else {
		sreg |=  (TPS2211_VPPD0 | TPS2211_VPPD1 |	/* VAVPP => Hi-Z */
			  TPS2211_VCCD0);			/* 3V on	*/
		sreg &= ~(TPS2211_VCCD1);			/* 5V off	*/
		DEBUG (0, " 3.3V card found: ");
	}
	...
	for (i=0; i<5000; ++i) {
		if ((cp->cp_pbdat & TPS2211_OC) == 0) {
		    printk ("   *** Overcurrent - Safety shutdown ***\n");
...
static void cfg_ports()
{
	...
	DEBUG (3, "Set Port C: PAR:     %04x DIR:     %04x DAT:     %04x\n",
	...
	DEBUG (3, "Set Port B: PAR: %08x DIR: %08x DAT: %08x\n",
...
/* PORT B */
			/* Output pins */
#define TPS2205_VCC5	0x00020000	/* PB.14:  5V Voltage Control	*/
#define TPS2205_VCC3	0x00000001	/* PB.31:  3V Voltage Control	*/
#define TPS2205_SHDN	0x00000200	/* PB.22: Shutdown		*/
#define TPS2205_PORTB_OUTPUTS ( TPS2205_VCC5    | TPS2205_VCC3    | \
			  TPS2205_SHDN)

			/* Input pins */
#define TPS2205_OC	0x00000100	/* PB.23: Over-Current		*/
#define TPS2205_PORTB_INPUTS	( TPS2205_OC )

/* PORT C */
			/* Output pins */
#define TPS2205_VPP_PGM	0x0020	/* PC.10: PGM Voltage Control	*/
#define TPS2205_VPP_VCC	0x0010	/* PC.11: VPP Voltage Control	*/
#define TPS2205_PORTC_OUTPUTS ( TPS2205_VPP_PGM  | TPS2205_VPP_VCC )

			/* Input pins */
#define TPS2205_PORTC_INPUTS	( 0 )
...
static int hardware_enable(int slot)
{
	...
	/* Configure Port C for TPS2205 PC-Card Power-Interface Switch */
	cfg_port_C ();
    ...
    reg = cp->cp_pbdat;
	shortreg  = immap->im_ioport.iop_pcdat;
	if ((pipr & mask) == mask) {
		shortreg |=  (TPS2205_VPP_PGM | TPS2205_VPP_VCC);	/* VAVPP => Hi-Z */
		reg |=	 TPS2205_VCC3;				/* 3V off	*/
		reg &= ~(TPS2205_VCC5);				/* 5V on	*/
		printk (KERN_INFO " 5.0V card found: \n");
	} else {
		shortreg |=  (TPS2205_VPP_PGM | TPS2205_VPP_VCC);	/* VAVPP => Hi-Z */
		reg |=   TPS2205_VCC5;				/* 5V off	*/
		reg &= ~(TPS2205_VCC3);				/* 3V on	*/
		printk (KERN_INFO " 3.3V card found: ");
	}
	...
	DEBUG (3, "\nPB DAT: %08x -> 3.3V %s 5.0V %s VPP_PGM %s VPP_VCC %s\n",
		...
		(shortreg & TPS2205_VPP_PGM) ? "off" : "on", 
		(shortreg & TPS2205_VPP_VCC) ? "off" : "on" );

    immap->im_ioport.iop_pcdat = shortreg;
	cp->cp_pbdat = reg;

	/*  Wait 500 ms; use this to check for over-current */
	for (i=0; i<5000; ++i) {
		if ((cp->cp_pbdat & TPS2205_OC) == 0) {
			printk (KERN_INFO "   *** Overcurrent - Safety shutdown ***\n");
...
static int voltage_set(int slot, int vcc, int vpp)
{
	...
	printk (KERN_INFO "PCMCIA powered %s\n", s);
    }
#endif
done:
	return (0);
...
static void cfg_port_B (void)
{
	 ...
	 */
	reg  = cp->cp_pbdat;
	reg |=  ( TPS2205_VCC3    | TPS2205_VCC5    |	/* VAVCC => Hi-Z */
              TPS2205_SHDN);				/* enable switch */
	cp->cp_pbdat = reg;

	cp->cp_pbpar &= ~(TPS2205_PORTB_INPUTS | TPS2205_PORTB_OUTPUTS);

	reg = cp->cp_pbdir & ~(TPS2205_PORTB_INPUTS);
	cp->cp_pbdir = reg | TPS2205_PORTB_OUTPUTS;
...
static void cfg_port_C (void)
{
	 ...
	 * Configure Port C for TPS2205 PC-Card Power-Interface Switch
	 *
	 * Switch off all voltages
	 */
	shortreg  = immap->im_ioport.iop_pcdat;
	shortreg |=  (TPS2205_VPP_PGM | TPS2205_VPP_VCC );	/* VAVPP => Hi-Z */
	immap->im_ioport.iop_pcdat = shortreg;

    immap->im_ioport.iop_pcpar &= ~(TPS2205_PORTC_INPUTS | TPS2205_PORTC_OUTPUTS);
    immap->im_ioport.iop_pcso  &= ~(TPS2205_PORTC_INPUTS | TPS2205_PORTC_OUTPUTS);

    shortreg = immap->im_ioport.iop_pcdir & ~(TPS2205_PORTC_INPUTS);
    immap->im_ioport.iop_pcdir = shortreg | TPS2205_PORTC_OUTPUTS;

	DEBUG (3,"Set Port C: PAR: %04x DIR: %04x DAT: %04x\n",
                  immap->im_ioport.iop_pcpar, immap->im_ioport.iop_pcdir, immap->im_ioport.iop_pcdat);
}
