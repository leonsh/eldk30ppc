...
#if defined(__PPC__) && defined(CONFIG_BLK_DEV_INITRD)
	if (initrd_start && !initrd_in_ram) {
		unsigned int initrd_size = initrd_end - initrd_start;
		initrd_start = (ulong)ioremap(initrd_start, initrd_size);
