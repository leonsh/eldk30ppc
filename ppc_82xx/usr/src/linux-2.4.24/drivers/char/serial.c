...
#if defined(CONFIG_DAB4K)
#if defined(MODULE)
/* load-time debug levels */
static int serial_debug_intr;
static int serial_debug_open;
static int serial_debug_flow;
static int serial_debug_rs_wait_until_sent;
static int serial_debug_autoconf;
static int serial_debug_mcount;
MODULE_PARM(serial_debug_intr, "i");
MODULE_PARM(serial_debug_open, "i");
MODULE_PARM(serial_debug_flow, "i");
MODULE_PARM(serial_debug_rs_wait_until_sent, "i");
MODULE_PARM(serial_debug_autoconf, "i");
MODULE_PARM(serial_debug_mcount, "i");
/* Turn on all compile-time debug code */
#define SERIAL_DEBUG_INTR 1
#define SERIAL_DEBUG_OPEN 1
#define SERIAL_DEBUG_FLOW 1
#define SERIAL_DEBUG_RS_WAIT_UNTIL_SENT 1
#define SERIAL_DEBUG_AUTOCONF 1
#define SERIAL_DEBUG_MCOUNT 1
#endif
static void dab4k_serial_init(void);
static void dab4k_serial_release(void);
/* 
 * We have to redefine this to not interfere with 8xx uart driver.
 */
#undef TTY_MAJOR
#undef TTYAUX_MAJOR
#define TTY_MAJOR 204
#define TTYAUX_MAJOR 205
#undef CONFIG_SERIAL_CONSOLE /* console is SMC/SCC uart */
...
#if defined(CONFIG_DAB4K)
#define DBG_CNT(s) if (serial_debug_mcount) \
printk("(%s): [%x] refc=%d, ttyc=%d -> %s\n", \
 kdevname(tty->device), (info->flags), serial_refcount, tty->count,s)
#else
...
#if defined(CONFIG_DAB4K) && defined(MODULE)
...
#if defined(CONFIG_DAB4K) && defined(MODULE)
		if (serial_debug_open || serial_debug_intr)
...
#if defined(CONFIG_DAB4K) && defined(MODULE)
				if (serial_debug_intr || serial_debug_flow)
					printk("CTS tx start...");
#else
				printk("CTS tx start...");
#endif
			 ...
			 struct termios *old_termios)
{
	...
	if (baud == 38400 &&
		...
		if (baud == 134)
			...
			quot = baud_base / baud;
#else
			/* Since the input clock is BRG, correct the divider
			 * to minimize the percent error.
			 */
...
#if defined(CONFIG_DAB4K) && defined(MODULE)
	if (serial_debug_rs_wait_until_sent) {
...
#ifdef CONFIG_SL8245
	{
		extern void sl8245_fix_uart (void);
...
#if defined(CONFIG_DAB4K)
	init_bh(IMMEDIATE_BH, do_serial_bh);
	dab4k_serial_init();
#else
...
#ifdef CONFIG_CU824
	/* On the CU824 board, there are two serial ports sharing one
	 * IRQ. This driver won't work if interrupts for both ports are
	 * enabled in the Local Interrupt Enable Register of the board.
	 * That's why we enable only one interrupt here, for the port
	 * that is going to be used as a serial console.
	 */
	*(volatile u_char *)(0xFE80000B) |= 0x20 >> co->index;
	*(volatile u_char *)(0xFE80000B) &= ~(0x10 << co->index);
#endif
...
#define OR_ACS_DIV4	0x00000400	/* CS is output 1/2 a clock later	*/
...
#define OR_SCY_6_CLK	0x00000060	/* 5 clock cycles wait states		*/
#define OR_SCY_7_CLK	0x00000070	/* 5 clock cycles wait states		*/
...
static void dab4k_serial_init(void)
{
	...
	/* Configure and setup BRG4 (PB28) */
	immap->im_cpm.cp_pbpar |= 0x00000008;
	immap->im_cpm.cp_pbdir &= ~0x00000008;
	immap->im_cpm.cp_pbodr &= ~0x00000008;

	cp->cp_brgc4 =
		(((((bd_t *)__res)->bi_intfreq / UART_BASE_BAUD) - 1) << 1) | 0x10000;
	
	/* Configure memory controller (GPCM) */
	/* Timing for 20Mhz input clock */
	memctl->memc_or3 = (BR_BA_MSK | OR_SCY_5_CLK | OR_BI);
	memctl->memc_br3 = ((UARTA_ADDR & BR_BA_MSK) | BR_PS_8 | BR_V);

	rs_table[0].iomem_base = ioremap(UARTA_ADDR, 0x100);
	rs_table[1].iomem_base = rs_table[0].iomem_base + 0x80;
}
...
static void dab4k_serial_release(void)
{
	...
	/* Mark the memory bank as invalid in GPCM */
	memctl->memc_br3 &= ~BR_V;

	iounmap(rs_table[0].iomem_base);
}
#endif
/*
  Local variables:
